...
- **A `tracer.iter` loop must not ask for a step the run does not make.** A bound the run meets keeps the code after the loop; one it does not meet raises `OutOfOrderError` naming the step asked for and the count reached. `max_new_tokens` is an upper bound — EOS, a stop string, or a `tracer.stop()` anywhere in the trace can end a run sooner — so pass `min_new_tokens=N` when a bound has to hold. An open `iter[:]` / `all()` ends *by* outrunning the run: it warns, keeps what the loop saved, and drops the statements after it — put those in a separate `tracer.invoke()`. `tracer.result` has to be read inside the block.
