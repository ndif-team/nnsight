...
- `code_reduce(source, globals, locals)` (`serialization.py`) is the core
  ...
  `_referenced_names(source)` (which compiles the source and walks
  ...
  of the payload.
- `reduce_block(node, globals, locals)` (`serialization.py`) reduces a
...
- `CustomCloudPickler(cloudpickle.CloudPickler)` (`serialization.py`) —
  overrides `_dynamic_function_reduce(func)` to serialize a dynamic
  `def`/`lambda` by source: `_lambda_source(func)` for lambdas, otherwise
...
- `persistent_id(obj)` (`serialization.py`) returns
  `obj.__dict__["_persistent_id"]` when present (else `None`).
- `CustomCloudUnpickler(pickle.Unpickler)` (`serialization.py`) overrides
  `persistent_load(pid)` to return `self.persistent_objects[pid]`, raising
  `UnknownPersistentIdError` if the ID is absent.

### Reconstruction

- `_load_function(...)` (`serialization.py`) rebuilds a function from a
...
- `_fill(func, globals)` (`serialization.py`) populates the function's globals
...
`_lambda_source` (`serialization.py`) AST-parses the line and disambiguates by
...
`dump`/`dumps` (`serialization.py`) and `load`/`loads`.
...
`RequestModel(BaseModel)` (`request.py`) is the JSON routing envelope. The
...
- `serialize(tracer, compress=False)` (`request.py`, classmethod) builds the
...
- `deserialize(blob, persistent_objects=None, compress=False, ...)`
  (`request.py`, staticmethod) is the inverse: `tracer, (source, glbls, vars) =
...
(`util.py`): a picklable stand-in carrying **only** code metadata
...
Related helpers in the same module: `Scope(dict)`, the 3-tier namespace a
captured block runs in (whose iteration yields only the block's own names — what
the reducers rely on); `push(frame, variables)`, which writes results
back into a live frame (`PyFrame_LocalsToFast` before 3.13, a plain `update` on
3.13+); and `clean_traceback`, which strips internal nnsight frames.

## Edits ride to the server — `Mediator.__getstate__`

An `edit` (from `model.edit(...)`) is a `Mediator` stored in `envoy._edits`.
`Mediator.__getstate__` (`interleaver.py`) serializes it cross-version-safely:
...
travel. `__setstate__` recompiles the source (under filename `<edit>`)
and re-inits.
...
- **Tagging (client):** `Envoy.__getstate__` (`envoy.py`) sets
  `interleaver._persistent_id = "Interleaver"` and `_module._persistent_id =
  f"Module:{self.path}"`. `TransformersModel.__getstate__` (`transformers.py`)
  additionally tags each non-`None` preprocessor and the pipeline, using the
  `_PERSISTENT` map (`transformers.py`): `tokenizer → "Tokenizer"`, `processor
...
- **Resolving (server):** `Remotable._remoteable_persistent_objects()`
  (`remotable.py`) builds the `{id: object}` map — `{"Interleaver":
  self.interleaver}` plus `Module:<path>` for every envoy in `self.modules()`.
  `TransformersModel` extends it (`transformers.py`) with the tokenizer and
...
`Remotable.to_model_key()` (`remotable.py`) is
...
`Status(str, Enum)` (`response.py`): `RECEIVED, QUEUED, PROVISIONING,
DEPLOYING, DISPATCHED, RUNNING, COMPLETED, ERROR, LOG`. `ResponseModel(BaseModel)`
carries `id`, `status`, `description`, and `data`. On `COMPLETED`, `data`
holds a presigned URL; the saves blob is downloaded separately and
`torch.load`-ed into a `RESULT = Dict[str, Any]`. `pickle`/`unpickle` use
`torch.save`/`torch.load` so tensors survive the wire.

## The `remote="local"` dry run — `backends/local.py`

`LocalSimulationBackend(Backend)` (`local.py`) exercises the entire remote path
...
`_SERVER_MODULES` (`local.py`) is the allowlist of roots assumed present on
...
1. **Client.** `model.trace(..., remote=True)` builds a `RemoteBackend`
   (`remotable.py`). On `__exit__`, `RequestModel.serialize(tracer)` reduces the
...
- `src/nnsight/intervention/serialization.py` — `code_reduce`,
  `reduce_block`, `_lambda_source`, `_load_function`,
  `_fill`, `CustomCloudPickler`, `CustomCloudUnpickler`,
  `dumps`/`loads`
- `src/nnsight/schema/request.py` — `RequestModel` (`serialize`/`deserialize`)
- `src/nnsight/schema/response.py` — `Status`, `ResponseModel`
- `src/nnsight/tracing/util.py` — `SerializedFrame`
- `src/nnsight/intervention/interleaver.py` — `Mediator.__getstate__`
- `src/nnsight/intervention/backends/local.py` — `LocalSimulationBackend`
