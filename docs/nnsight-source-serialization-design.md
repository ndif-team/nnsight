...
When `st.get_hidden(10)` executes inside a trace, it doesn't actually retrieve hidden states—it builds up a symbolic computation graph that will be executed on the server. The `StandardizedTransformer` class is essentially a query generator, not runtime code.
...
The server doesn't need nnterp installed—it just needs the source code of the specific classes being used.
