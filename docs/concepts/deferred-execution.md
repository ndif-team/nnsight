...
one_liner: A trace block is captured by source, compiled into a standalone function, and run interleaved with the model — parking in a greenlet on each Envoy property access until the module's controller hands the value over.
...
sources: [src/nnsight/tracing/tracer.py, src/nnsight/intervention/tracer.py, src/nnsight/intervention/envoy.py]
---
...
The plumbing lives in the base `Tracer` (`tracing/tracer.py`); `InterleavingTracer` (`intervention/tracer.py`) only overrides how the captured body is run.
...
2. `__enter__` (`tracing/tracer.py`) calls `capture()`:
...
4. Python unwinds the `with`. `__exit__` (`tracing/tracer.py`) catches `ExitTracingException`, increments the trace-depth, and calls `self.backend(self)` — which calls `tracer.execute(code)`.
5. `InterleavingTracer.execute` (`intervention/tracer.py`) sets up a `Batcher`, then:
...
6. `Envoy.interleave` (`envoy.py`) enters the shared `Interleaver` context. `Interleaver.__enter__` `start()`s every mediator's greenlet worker, running each up to its first park. Then `fn(*combined_input)` runs the model on the main greenlet.
7. As the model reaches each module, its controller calls `Interleaver.handle(location, value)`, which offers the value to any worker parked on that location — serving reads, applying swaps — and returns the (possibly edited) value back into the forward.
...
By default nothing a trace computes leaves it. `save(value)` (`tracing/tracer.py`, exposed as `nnsight.save` and `.save()`) marks a value **by identity** to survive past the *outermost* trace.
...
`model.scan(x)` uses `ScanningTracer` (`intervention/tracer.py`), which runs the same interleave under `FakeTensorMode`: operations propagate only shape/dtype/device, no real compute, no dispatch. Read `.shape`/`.dtype` inside the block and save those — a fake tensor saved out of the block keeps answering tensor operations with more fake tensors, and only raises when something asks it for a real number. See `docs/usage/scan.md`.

## Gotchas

- **The body never runs in your file's namespace directly.** It runs as a compiled function against a `Scope` (a snapshot of your locals + the live frame + globals; see `tracing/util.py`). Assignments flow back via `push_result`, filtered to saved values at the outermost trace.
