...
- [Interleaver and Hooks](interleaver-and-hooks.md) — One shared `Interleaver` installs a controller forward on every module at wrap time; it passes through when not interleaving and hands off through `Interleaver.handle` when it is. Occurrence tagging (`.i{n}`) and out-of-order detection.
