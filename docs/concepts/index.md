...
- [Interleaver and Hooks](interleaver-and-hooks.md) — One shared `Interleaver` installs a pre-forward and forward hook on every module at wrap time; they pass through when not interleaving and route through `Interleaver.handle` when they are. Occurrence tagging (`.i{n}`) and out-of-order detection.
