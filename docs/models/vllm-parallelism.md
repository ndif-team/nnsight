...
`VLLMFragments` (`fragments.py`) gathers a `ColumnParallelLinear`/`RowParallelLinear` shard into the full tensor before your intervention reads it and re-splits on write, so every rank runs the same code on the same complete tensor: `qkv_proj.output`, `gate_up_proj.output`, `o_proj.input` and `down_proj.input` read at their full width; layer outputs and `norm.output` are whole on every rank already. Parameters are not: `layer.weight` is this rank's slice.

The two column-parallel layers are *fused* — `qkv_proj` packs Q/K/V into one weight, `gate_up_proj` packs gate/up — and each rank holds a slice of every packed part, so the ranks' bare concatenation would be `[q₀ k₀ v₀ | q₁ k₁ v₁]` rather than `[q | k | v]`. nnsight un-interleaves it on the way in and re-interleaves on write, so a column means the same feature at every `tensor_parallel_size`: `gate, up = gate_up_proj.output[0].chunk(2, -1)` and a slice by head are the code you would write at `tp=1`. (With fewer KV heads than ranks vLLM replicates K and V across a group of ranks; the gathered value carries each of them once, at the single-rank width.) Verified in `tests/vllm/test_tensor_parallel.py` against a one-rank engine, element-wise.
