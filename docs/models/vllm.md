...
- **A tap can be an operation inside a forward.** `"model.layers.10.self_attn.source.qkv_split_0.output"` taps a `.source` op: the worker installs the source-instrumented forward for exactly the modules such taps name, before the graphs are recorded, and the op is served on replay like any module location (reads bitwise-equal to the eager engine, in-place edits land). An op name the forward does not have is a load error listing the ones it has. Ops inside a fused kernel are still not locations.
