...
def _submit_via_serve(client_model, prompt, url):
    ...
    """
    with client_model.trace(prompt, serve=url):
...
class TestCorrectness:
    ...
    def test_concurrent_results_nontrivially_correlated_with_sequential(
        ...
        This is a loose check because the two paths capture different
        forward passes. Serve runs ``SERVER_MAX_NEW_TOKENS`` forwards
        per request (one prefill + 19 decodes). The user's
        ``.save()`` fires on each forward and overwrites the stored
        value, so ``logits`` ultimately captures the LAST decode step
        — shape ``(1, 1, vocab)``. Local sequential runs one forward
        and captures ``(1, prompt_len, vocab)``. The test compares
        the trailing ``vocab`` slice of each, so they're pointing at
        different distributions (last-decode-step vs last-prompt-
        token). Cosine similarity varies per prompt depending on how
        informative the last decoded token is. We require that
        *on average* across prompts the correlation is positive,
        which is a weak sanity rail against wiring-level regressions
        (wrong model, wrong weights, wrong request routing).

        Note: an earlier version of this docstring incorrectly
        attributed the weakness to the hardcoded ``num_tokens_map=1``
        in ``_step``. That hardcoding is actually correct for
        vanilla's ``[batch_size, max_input_len, hidden]`` layout,
        where each request is one row and ``batch_group=[row_idx, 1]``
        is the right slice descriptor. The real cause is the save-
        overwrite-across-decode-steps semantics noted above.
        """
        ...
        # Tightening this bound would require either comparing
        # matched forward passes (e.g. client sets max_new_tokens=1
        # and trims the baseline to the prefill forward too) or
        # comparing full generated sequences instead of a snapshot
        # logits tensor. Both are structural test changes, not
