...
def scenario_write_cross(model, args):
    ...
    # (stage 1). On the owning rank the early value arrives from stage 0
    # while the block is parked inside the late layer's hook.
    with model.trace(args.prompt, temperature=0.0, max_tokens=1):
        # Clone at read time: holding the live activation across the layers
        # between read and write lets vLLM's buffer reuse mutate it. The
        # value that crosses stages is a copy, so without this snapshot PP=1
        # (mutated buffer) and PP=2 (clean copy) graft different values.
...
def scenario_multigen_forced(model, args):
    # The norm() consumes the cross-stage value on the non-owning rank at
    # every step: one pinned read per round, parked again for the next round
    # from inside the step-start serve.
    import nnsight
