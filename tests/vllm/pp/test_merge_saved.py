...
def test_merge_collected_assembles_stage_payloads():
    ...
    assert merge_collected([None, None]) == {}


def test_merge_collected_keeps_the_earliest_rank_for_whole_values():
    rank0 = pickle.dumps({"req": {"saves": {"x": torch.ones(2)}, "error": None}})
    rank1 = pickle.dumps({"req": {"saves": {"x": torch.ones(2) * 5}, "error": None}})
    merged = merge_collected([rank0, rank1])
    assert torch.equal(merged["req"]["saves"]["x"], torch.ones(2))


def test_merge_collected_unions_caches_in_the_sequence_saves():
    from nnsight.intervention.cache import Cache, CacheView, Entry

    def view(entries):
        cache = Cache.__new__(Cache)
        cache.model, cache.targets, cache.entries = None, None, entries
        return CacheView(cache, None)

    def payload(view_):
        return pickle.dumps({"req": {
            "saves": {"cache": view_}, "error": None,
            "sequences": {0: {"saves": {"cache": view_}, "registered": {}}},
        }})

    stage0 = payload(view({"model.layers.2": [Entry(output=torch.ones(1))]}))
    stage1 = payload(view({"model.layers.20": [Entry(output=torch.zeros(1))]}))
    merged = merge_collected([stage0, stage1])
    for cache in (merged["req"]["saves"]["cache"], merged["req"]["sequences"][0]["saves"]["cache"]):
        assert set(cache._cache.entries) == {"model.layers.2", "model.layers.20"}
