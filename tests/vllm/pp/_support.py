...
* ``test_unit_*``: one process, no GPU. Ownership, shells, the wire codec.
* ``test_harness_*``: two gloo ranks over the real ``PPInterleaver`` and
  ``Link``, no GPU and no vLLM engine. `Stage` opens one rank's half of that
  harness; `run_two_ranks` spawns both.
...
# A cross-stage take that cannot complete times out after the link's deadline
# (60s); a scenario that finishes well inside that bound did not stall on one.
STALL_BOUND_S = 40.0
...
# Longer than any wait the harness can make a rank do (the harness link times
# out at a quarter of this); a run past it is a deadlock and is torn down.
...
class Stage:
    """One rank's half of the two-rank harness: a `Link` over the world group
    and, given ``owners``, a `PPInterleaver` whose module map they define.

    Attributes:
        rank, world: This rank and the world size.
        link: The rank's end of the wire.
        interleaver: The PPInterleaver, or ``None`` for a wire-level test.
    """

    def __init__(self, rank: int, world: int, rdv: str, owners: Optional[dict] = None, resolver: Any = None, **interleaver_kwargs: Any) -> None:
        from nnsight.modeling.vllm.pp import PPModuleMap
        from nnsight.modeling.vllm.pp_interleaver import PPInterleaver
        from nnsight.modeling.vllm.pp_transport import Link

        dist.init_process_group("gloo", init_method=f"file://{rdv}", rank=rank, world_size=world)
        self.rank, self.world = rank, world
        self.link = Link(dist.group.WORLD, rank, world, resolver=resolver, timeout=HARNESS_TIMEOUT_S / 4)
        self.interleaver = None
        if owners is not None:
            module_map = PPModuleMap(world)
            module_map.set_derived_owners(owners)
            self.interleaver = PPInterleaver(module_map, self.link, rank, torch.device("cpu"), **interleaver_kwargs)

    def mediator(self, block: str, req_id: str = "r", ordinal: int = 0, start: bool = True):
        """A worker for ``block`` (which reads through ``Mediator.value``) on this
        rank's interleaver, stamped as the runner stamps one, started and chased."""
        ...
        if start:
            ...
            self.interleaver.started(mediator)
            self.interleaver.mediators.append(mediator)
            self.interleaver.reindex()
        return mediator

    def fire(self, provider: str, value: Any) -> Any:
        """The model reaching ``provider`` on this rank."""
        result = self.interleaver.handle(provider, value)
        self.interleaver.reindex()
        return result
