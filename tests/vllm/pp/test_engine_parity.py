...
cross-stage reads. It pins per-request narrowing of what is pushed,
request-keyed inboxes, and both invokes' values staying distinct.
"""
...
def test_cross_stage_read_modify_write():
    # The graft needs an earlier stage's value while parked inside the late
    # layer's hook on the owning rank; it is taken in place, so the swap
    # that follows still lands before the forward moves on.
...
def test_multi_token_forced_cross_stage_reads_match():
    # Consumes the late layer's value on every generation step, so the
    # non-owning rank takes one pinned read per round and parks on the next
    # round's from inside the step-start serve.
