...
The :class:`Graph <nnsight.tracing.Graph.Graph>` class adds and stores each operation as a Node.

The :class:`Node <nnsight.tracing.Node.Node>` class represents nodes in the graph. The class represents the operations (and the resulting output of said operations) they are tracing AND nodes that actually execute the operations when executing the Graph.
