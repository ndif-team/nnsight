...
class eproperty:
    ...
    def __get__(self, envoy: Envoy, owner):
        ...
        if envoy.interleaving:
            ...
            if requester in envoy._interleaver.current.history:
...
def input_hook(fn: Callable[..., T]) -> Callable[..., T]:

    @wraps(fn)
    def wrapper(self: Envoy, *args, **kwargs):
        self._interleaver.hooks[f"{self.path}.input"][1].enabled = True
        return fn(self, *args, **kwargs)

    return wrapper


def output_hook(fn: Callable[..., T]) -> Callable[..., T]:

    @wraps(fn)
    def wrapper(self: Envoy, *args, **kwargs):
        self._interleaver.hooks[f"{self.path}.output"][1].enabled = True
        return fn(self, *args, **kwargs)
...
class Envoy(Batchable):
    ...
    def skip(self, replacement: Any):
        ...
        """
        if f"{self.path}.input" in self._interleaver.current.history:
            raise Mediator.OutOfOrderError(
                f"Value was missed for {f"{self.path}.input"}. Did you call an Envoy out of order?"
            )
