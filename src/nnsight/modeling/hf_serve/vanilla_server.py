...
class VanillaBatchServer:
    ...
    def build_entries(
        ...
        Args:
            mediators: Explicit mediator list. The HTTP handler passes
                ...
                racing with the bg thread on ``model.interleaver``;
                that path cannot rely on ``model.interleaver.mediators``
    ...
    def _step(self, scheduled: List[ScheduledItem]):
        ...
        if has_mediators:
            ...
            # ``deferred_exception`` after the forward pass and finalize
