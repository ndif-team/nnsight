...
* **Publish** (producer side): as ``handle`` serves a location the rank owns,
  each served request's rows of the post-intervention value are cloned into
  the pull buffer under ``("{provider}.i{visit}", req_id)`` and any parked
  peer pulls for that key are dispatched (the base interleaver's ``publish``
  seam). It runs inside ``handle``'s fragments bracket, so under TP within a
  stage the published value is the assembled whole, matching what a local
  worker read.
...
rank (which tags by the worker's own occurrence of the location) and a
non-owning rank (whose forward never visits it). The non-owning side tags a
pinned read (``tracer.iter``) with the pinned step, and a relaxed read with the
request's completed-round count, which equals the occurrence for modules the
forward reaches once per step. A module visited several times in one forward is
mis-tagged on the non-owning side.
"""
...
from ...intervention.interleaver import STEP_GATE, Event, Mediator
...
class PPInterleaver(VLLMInterleaver):
    ...
    def __init__(
        ...
        # `handle` assembles the whole before the workers are served, so both
        # the local workers and the publish see the real tensor. The vLLM
        # runner serves the step gate at its own boundary, once per engine
        # step.
        super().__init__(taps, fragments=fragments, step_gate_at_root=False)
    ...
    def publish(self, provider: str, value: Any, served: list) -> None:
        # Publish exactly what some worker was served this visit: every rank
        # runs the same block, so a location a peer pulls is one this rank's
        # copy of the worker parked on locally, and ``served`` carries those
        # workers with the occurrence they waited for. The value arrives before
        # any re-split for the model: under TP within a stage it is the
        # assembled whole a local worker saw. The step gate carries no data,
        ...
        if (
            ...
            self._publish(provider, value, served)
    ...
    def _publish(self, provider: str, value: Any, visits: list) -> None:
        ...
        for mediator, visit in visits:
            ...
            # Copy to host, here on the forward thread: the buffer must hold the
            # value as of this visit (the model or a later intervention may
            # mutate the live tensor), and the copy is ordered on the forward's
            # stream at this point of the forward, ahead of the pipeline's own
            # transfer of this step's activations. The wire carries host
            # tensors, so the reply thread ships the buffered value as is.
            cloned = tree_map(
                lambda t: t.detach().to("cpu", copy=True) if isinstance(t, torch.Tensor) else t,
                served,
            )
            key = (tagged, req_id)
            with condition:
        ...
        while progressed:
            ...
            for mediator in (
                ...
                if not mediator.alive or mediator.pending is None:
                    continue
                if mediator.pending.event is not Event.VALUE:
                    continue
                # The park's provider is the encoded pull location itself.
                untagged = mediator.pending.provider
    ...
    def has_pending_pulls(self) -> bool:
        ...
        for mediator in self.mediators:
            if (
                ...
                and mediator.pending.provider is not None
                and mediator.pending.provider.startswith(
                    PULL_LOCATION_PREFIX
                )
            ):
                return True
