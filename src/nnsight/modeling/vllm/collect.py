"""Assemble per-request save payloads into results.

Two merges live here. :func:`merge_shared_saves` reproduces the shared-object
semantics of a multi-invoke trace: locally the invoke blocks of one trace share
their outer names (a container bound above the invokes is ONE object every
invoke mutates), while on the engine each invoke rides its own request with its
own COPY, so the copies come back one per request and are merged before results
are pushed back into the trace frame.

:func:`merge_saved` is the pipeline-parallel merge. Every PP stage runs the
intervention body and ships a partial save tree from its TP-rank-0: the slots
it owns are real values, foreign slots are
:data:`~.lazy_remote_tensor.NOT_ON_THIS_RANK` sentinels (a saved
LazyRemoteTensor strips to one before shipping, see :func:`strip_lazy`). The
organizing principle is stage ownership, encoded by the sentinel: preferring
the real leaf over the sentinel is trusting the owning rank. A non-owning rank
that holds a real copy of a model-derived value materialized the owner's, so
two real copies of anything model-derived are equal; only model-independent
values (in-trace RNG, per-rank environment values) can differ, and those raise
...
def strip_saves(values: dict) -> dict:
    """A request's saved names as one stage ships them.

    Lazies inside a value strip to sentinels for the engine-side merge, and a
    name whose value is purely another stage's is left out: the owning stage
    ships the real data.
    """
    shipped = {}
